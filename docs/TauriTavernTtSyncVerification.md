...
deploy report 必須來自不使用 `--allow-placeholders` 的真實 env 驗證，且 direct Node `ExecStart`、hardening、public URL、port、non-placeholder pairing token 與 data dir writable path 等必要檢查都必須是 `ok=true`；deploy report 的 `publicUrl` 必須和遠端 smoke endpoint 一致。
遠端 smoke report 必須包含 `completedAt`、`smokePath`、`deviceId`、push/pull `planIds`、`status.version`、fixture files/bytes 與 smoke paths，且必要 smoke checks 都必須是 `ok=true`。
