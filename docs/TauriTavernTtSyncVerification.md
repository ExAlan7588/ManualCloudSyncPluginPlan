...
deploy report 的 `publicUrl`、遠端 smoke endpoint 與 device evidence 的 `server.url` 都不能使用 `example.com` / `example.net` / `example.org` 或 `*.example.com` 這類 example placeholder domains。
