...
5. 可先用 `npm run evidence:device-template -- --output <device-evidence.json>` 建立真機證據模板；模板預設 `ok=false`，不會被 final gate 視為完成。
6. 使用 `npm run verify:incremental-evidence -- --commands <command-report.json> --smoke <remote-smoke-report.json> --device-evidence <device-evidence.json>` 做最終證據 gate。
