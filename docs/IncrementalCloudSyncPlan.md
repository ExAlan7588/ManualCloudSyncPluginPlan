...
- 手機/電腦 app build command evidence 已用 `/tmp/TauriTavern-inspect` 的 Android `x86_64` universal debug APK 與 Linux desktop release binary 產生並通過 verifier；Android clean APK 也已在 Android 15 emulator 載入 TauriTavern 基礎 UI，證據位於 `/tmp/tt-sync-android-runtime-evidence-run5`。Android runtime command 證據位於 `/tmp/tt-sync-android-command-probe`、`/tmp/tt-sync-android-sync-probe`、`/tmp/tt-sync-android-weak-network-probe` 與公開配對 artifacts：透過 WebView Chrome DevTools Protocol 呼叫真實 `window.__TAURI__.core.invoke('tt_sync_pair')` / `tt_sync_list_servers`，並在 app 重啟後確認 `paired-servers.json` 保存公開 Cloudflare endpoint `https://alex-coupons-herbs-folding.trycloudflare.com`、server `335ae38b-bb32-4cab-a8ca-dd95e4bcc7d5` 與 SPKI `PBOUgdwaUmnxFCRJ-mucYGFOFq7_E-X4e40iuR8zVwI`；公開配對證據位於 `/tmp/tt-sync-public-android-cloudflare-pair-result.json`、`/tmp/tt-sync-public-android-cloudflare-list-after-restart.json` 與 `/tmp/tt-sync-public-android-cloudflare-paired-servers.json`。Android runtime 也呼叫真實 `tt_sync_push` / `tt_sync_pull`，捕捉 `tt_sync:progress`、`tt_sync:completed` 與弱網路下的 `tt_sync:error`，其中 push 完成 164 檔 / 7,858,061 bytes，airplane-mode 斷線時 UI 顯示 `Internal error: error sending request for url (https://10.0.2.2:9443/v2/session/open)`。Linux desktop runtime 證據位於 `/tmp/tt-sync-desktop-runtime-probe` 與 `/tmp/tt-sync-desktop-mtime-probe`：在 Docker/Xvfb 內透過 WebKit HTTP inspector 呼叫真實 `window.__TAURI__.core.invoke('tt_sync_pair')` 與 `tt_sync_list_servers`，並在重啟後確認公開 Cloudflare endpoint 同樣保存為 server `335ae38b-bb32-4cab-a8ca-dd95e4bcc7d5`；公開 desktop artifacts 位於 `/tmp/tt-sync-public-desktop-cloudflare-pair-result.json`、`/tmp/tt-sync-public-desktop-cloudflare-list-after-restart.json` 與 `/tmp/tt-sync-public-desktop-cloudflare-paired-servers-after-restart.json`。另已用真實 `tt_sync_pull` 確認 remote file 寫入後本機 `mtime` 與 `modifiedMs` 完全一致。LAN/cloud mutex runtime 證據也已補齊：Android 端與 desktop LAN server 配對後，`tt_sync_push` Uploading 期間呼叫 `lan_sync_sync_from_device` 會顯示 `LAN sync already running`，`lan_sync_sync_from_device` Downloading 期間呼叫 `tt_sync_pull` 會顯示 `TT-Sync already running`，證據位於 `/tmp/tt-sync-android-weak-network-probe/lan-mutex-cloud-active-64mb-rerun.json`、`/tmp/tt-sync-android-weak-network-probe/lan-mutex-lan-active-128mb.json` 與 `/tmp/tt-sync-desktop-runtime-probe/lan-mutex`。
- Minimal TT-Sync server 已在 repo 內建立並以自動測試驗證；systemd/env 設定可用 `npm run verify:tt-sync-deploy` 檢查；`/tmp/tt-sync-public-deploy.json` 與 `/tmp/tt-sync-public-smoke.json` 已使用同一個 Cloudflare HTTPS endpoint `https://alex-coupons-herbs-folding.trycloudflare.com`，smoke report 以 `namespace=default` 產生與 Tauri runtime 相同的 server id `335ae38b-bb32-4cab-a8ca-dd95e4bcc7d5`。大資料 first-sync push 已用 HTTPS tunnel commit 335,544,320 bytes。
- 真實裝置端到端首同步與 mirror delete 驗證仍需補齊；Android emulator 已取得基礎 UI、public pair persistence、runtime push/pull command、LAN/cloud 互斥與弱網路可見錯誤證據，Linux desktop 也已取得 public pair persistence、pull mtime 與 pull interruption 證據。更新後 `npm run verify:incremental-evidence` 只剩 pre-transfer diff summary 與 conflict resolution 的實際 UI evidence 尚未通過。
...
4. 帳號登入後可產生短效一次性配對 URI，讓 App 後端保存裝置同步授權。
5. UI 顯示各裝置最後同步時間與差異。
6. 支援衝突檢視。
...
- [x] 手機與電腦能保存配對服務端。Android emulator 與 Linux desktop 都已用公開 Cloudflare HTTPS endpoint `https://alex-coupons-herbs-folding.trycloudflare.com` 執行真實 `tt_sync_pair`，並在 app 重啟後以 `tt_sync_list_servers` 或 `paired-servers.json` 確認保存同一 server `335ae38b-bb32-4cab-a8ca-dd95e4bcc7d5`；證據為 `/tmp/tt-sync-public-android-cloudflare-list-after-restart.json` 與 `/tmp/tt-sync-public-desktop-cloudflare-list-after-restart.json`。
...
- [x] 帳號登入後產生短效配對 URI。結果：`POST /v2/account/pairing-uri` 會用 account bearer token 產生 10 分鐘一次性 `tauritavern://tt-sync/pair?...` URI，不在回應中重複裸露 token；完成配對後 token 會被消耗。
...
- [x] Push/Pull 操作按鈕。
- [x] 帳號登入與短效配對 URI UI。結果：插件 `帳號式同步服務` 區塊可登入 Minimal server、產生配對 URI、填入既有配對欄位，並查詢裝置與同步歷史；帳號 token 只保存在目前頁面記憶體。
- [x] 進度欄位顯示。
...
- [x] LAN Sync 與雲端同步不能並行。結果：Android emulator 與 Docker/Xvfb desktop LAN server 完成真實 LAN pairing；`tt_sync_push` 進入 Uploading 後觸發 `lan_sync_sync_from_device`，WebView event 與 UI body 顯示 `Invalid data: LAN sync already running`；`lan_sync_sync_from_device` 進入 Downloading 後觸發 `tt_sync_pull`，WebView event 與 UI body 顯示 `Invalid data: TT-Sync already running`。證據：`/tmp/tt-sync-android-weak-network-probe/lan-mutex-cloud-active-64mb-rerun.json`、`/tmp/tt-sync-android-weak-network-probe/lan-mutex-lan-active-128mb.json`、兩張 error screenshot 與 `/tmp/tt-sync-device-evidence-current-audit.json` 的 `lanCloudSyncMutex.ok=true`。
...
- 衝突第一版採保守策略：Minimal server 會在 unresolved conflict 時阻止破壞性 commit；插件已準備好渲染真實 `tt_sync:conflict` payload，並在卡片上提供本機 / 遠端決策按鈕；更新後的 TauriTavern source tree 也已補上 conflict DTO 與決策 payload surface。剩餘外部缺口是實際 app UI 中 pre-transfer diff summary 與 conflict resolution 可見證據。
