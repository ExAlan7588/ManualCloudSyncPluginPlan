...
function commandReportReferenceChecks(options) {
    ...
    return [
        check(
            `same ${options.target} command report tool`,
            sameText(options.report?.tool, reference?.tool),
            `${options.target} command report tool and device evidence reference must match`,
        ),
        check(
            `same ${options.target} command report schema version`,
            sameNumber(options.report?.schemaVersion, reference?.schemaVersion),
            `${options.target} command report schemaVersion and device evidence reference must match`,
        ),
        check(
