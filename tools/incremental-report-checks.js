...
export function deployChecks(report) {
    ...
    return [
        ...deployReportShapeChecks(report),
        ...deployPathChecks(report),
        ...deployPublicUrlChecks(report),
        ...deployReportContentChecks(report, names),
    ];
}
...
export function smokeChecks(report) {
    const names = passedCheckNames(report);
    return [
        ...smokeReportShapeChecks(report),
        ...smokeIdentityChecks(report),
        ...smokeFixtureChecks(report),
        ...smokeEndpointChecks(report),
        ...smokeReportContentChecks(report, names),
    ];
}
...
function deployReportShapeChecks(report) {
    return [
        check('deploy report ok', report?.ok === true, 'deploy report must have ok=true'),
        check('deploy report tool', report?.tool === 'verify-tt-sync-deploy', 'deploy report must be generated by verify-tt-sync-deploy'),
        check('deploy report schema version', report?.schemaVersion === REPORT_SCHEMA_VERSION, 'deploy report must use schemaVersion=1'),
        check('deploy report verifiedAt', isTimestamp(report?.verifiedAt), 'deploy report must include a parseable verifiedAt timestamp'),
    ];
}

function deployPathChecks(report) {
    return [
...
function deployPublicUrlChecks(report) {
    return [
...
function deployReportContentChecks(report, names) {
    return [
...
function smokeReportShapeChecks(report) {
    return [
        check('smoke report ok', report?.ok === true, 'smoke report must have ok=true'),
        check('smoke report tool', report?.tool === 'smoke-tt-sync-server', 'smoke report must be generated by smoke-tt-sync-server'),
        check('smoke report schema version', report?.schemaVersion === REPORT_SCHEMA_VERSION, 'smoke report must use schemaVersion=1'),
...
function smokeIdentityChecks(report) {
    return [
...
function smokeFixtureChecks(report) {
    return [
...
function smokeEndpointChecks(report) {
    return [
...
function smokeReportContentChecks(report, names) {
    return [
