...
export function incrementalEvidenceChecks(evidence) {
    return [
...
export function evidenceReportFor(checks) {
...
export function formatCheckLine(checkItem) {
    return `- ${checkItem.ok ? 'ok' : 'missing'} ${checkItem.name}: ${checkItem.detail}`;
}

function commandChecks(report, target) {
    return [
        check(`${target} command report tool`, report?.tool === COMMAND_REPORT_TOOL, `${target} command report must be generated by ${COMMAND_REPORT_TOOL}`),
        check(`${target} command report schema version`, report?.schemaVersion === COMMAND_REPORT_SCHEMA_VERSION, `${target} command report must use schemaVersion=1`),
...
function eventChecks(report) {
    ...
    return [
        ...eventReportShapeChecks(report),
        ...eventReportSourceChecks(report),
        ...eventReportContentChecks(report, names),
    ];
}

function eventReportShapeChecks(report) {
    return [
        check('event surface report tool', report?.tool === EVENT_SURFACE_REPORT_TOOL, `event surface report must be generated by ${EVENT_SURFACE_REPORT_TOOL}`),
        check('event surface report schema version', report?.schemaVersion === EVENT_SURFACE_REPORT_SCHEMA_VERSION, 'event surface report must use schemaVersion=1'),
...
function eventReportSourceChecks(report) {
    return [
...
function eventReportContentChecks(report, names) {
    return [
...
function commandReportReferenceChecks(options) {
    ...
    return [
        check(`same ${options.target} command report tool`, sameText(options.report?.tool, reference?.tool), `${options.target} command report tool and device evidence reference must match`),
        check(`same ${options.target} command report schema version`, sameNumber(options.report?.schemaVersion, reference?.schemaVersion), `${options.target} command report schemaVersion and device evidence reference must match`),
        check(`same ${options.target} command report source`, sameText(options.report?.source, reference?.source), `${options.target} command report source and device evidence reference must match`),
        check(`same ${options.target} command report scannedAt`, sameText(options.report?.scannedAt, reference?.scannedAt), `${options.target} command report scannedAt and device evidence reference must match`),
        check(`same ${options.target} command report sourceKind`, sameText(options.report?.sourceKind, reference?.sourceKind), `${options.target} command report sourceKind and device evidence reference must match`),
    ];
}
...
function eventReportReferenceChecks(evidence) {
    ...
    return [
        check('same event report tool', sameText(evidence.eventReport?.tool, reference?.tool), 'event surface report tool and device evidence reference must match'),
        check('same event report schema version', sameNumber(evidence.eventReport?.schemaVersion, reference?.schemaVersion), 'event surface report schemaVersion and device evidence reference must match'),
...
function serverConsistencyChecks(evidence) {
    return [
        check(
            ...
            integerAtLeast(deviceCheckValue(evidence, 'realLargeFirstSyncCompleted', 'metrics.totalBytes'), MIN_REAL_LARGE_SYNC_BYTES),
            'realLargeFirstSyncCompleted.metrics.totalBytes must be at least 300MiB',
        ),
        check('phone saved server URL matches', sameEndpoint(deviceCheckValue(evidence, 'phoneDesktopPairingSaved', 'phone.savedServerUrl'), evidence.deviceEvidence?.server?.url), 'phoneDesktopPairingSaved.phone.savedServerUrl must match device evidence server.url'),
        check('desktop saved server URL matches', sameEndpoint(deviceCheckValue(evidence, 'phoneDesktopPairingSaved', 'desktop.savedServerUrl'), evidence.deviceEvidence?.server?.url), 'phoneDesktopPairingSaved.desktop.savedServerUrl must match device evidence server.url'),
        check('phone saved server id matches', sameText(deviceCheckValue(evidence, 'phoneDesktopPairingSaved', 'phone.savedServerId'), evidence.smokeReport?.serverId), 'phoneDesktopPairingSaved.phone.savedServerId must match smoke report serverId'),
        check('desktop saved server id matches', sameText(deviceCheckValue(evidence, 'phoneDesktopPairingSaved', 'desktop.savedServerId'), evidence.smokeReport?.serverId), 'phoneDesktopPairingSaved.desktop.savedServerId must match smoke report serverId'),
        check('deploy and smoke same URL', sameEndpoint(evidence.deployReport?.publicUrl, evidence.smokeReport?.endpoint), 'deploy publicUrl and smoke endpoint must match'),
        check('same server URL', sameEndpoint(evidence.smokeReport?.endpoint, evidence.deviceEvidence?.server?.url), 'smoke endpoint and device evidence server.url must match'),
    ];
}
