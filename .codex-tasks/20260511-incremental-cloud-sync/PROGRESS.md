...
- 形态: single-full
- 进度: 406/406
- 当前: account-style sync service phase committed. Minimal server can mint one-time account-authenticated TauriTavern pairing URIs, consume them on pair completion, serve CORS for the Luker-hosted panel, and the plugin now has account login, token refresh, pairing URI generation, devices, and history UI.
- 文件: `.codex-tasks/20260511-incremental-cloud-sync/TODO.csv`
- 下一步: run the completion audit against the active objective.
...
- 2026-05-12: Audited the LAN/cloud mutex runtime path. Source inspection shows LAN Sync and TT-Sync share the same `Arc<Semaphore>` from bootstrap, `lan_sync_sync_from_device` and `tt_sync_pull`/`tt_sync_push` acquire it before transfer work, and both runtimes emit visible error events on busy permits. Runtime probing also confirms the current desktop WebKit inspector can execute wrapped `Runtime.evaluate` against target `page-7`, while Android WebView command injection remains available.
- 2026-05-12: Captured bidirectional LAN/cloud mutex runtime evidence. Android paired with the Docker/Xvfb desktop LAN server at `http://172.17.0.3:63032`; while `tt_sync_push` was in Uploading, `lan_sync_sync_from_device` emitted `Invalid data: LAN sync already running`, and while `lan_sync_sync_from_device` was in Downloading, `tt_sync_pull` emitted `Invalid data: TT-Sync already running`. Evidence artifacts are `/tmp/tt-sync-android-weak-network-probe/lan-mutex-cloud-active-64mb-rerun.json`, `/tmp/tt-sync-android-weak-network-probe/lan-mutex-lan-active-128mb.json`, matching screenshots, and `/tmp/tt-sync-desktop-runtime-probe/lan-mutex`.
- 2026-05-12: Updated `/tmp/tt-sync-device-evidence-current-audit.json` so `lanCloudSyncMutex.ok=true`; rerunning the final evidence gate drops the LAN/cloud mutex failure and leaves 7 failures: public endpoint phone/desktop pairing URL/id consistency, pre-transfer diff UI, and conflict resolution UI.
- 2026-05-12: Validation passed for the LAN/cloud mutex documentation phase: `npm run check`, `node tools/test/docs-coverage-run-tests.js`, `git diff --check`, and `timeout 60s npm test -- --runInBand` all passed. The final evidence gate was rerun and remains intentionally failing on the 7 non-mutex blockers.

- 2026-05-13: User requested no more real-App Docker testing because an unbounded Docker mount exhausted VPS resources. Scope changed to PM2 Luker only, with no Docker, no large sync, no full data scan, and no real Tauri App runtime probing.
- 2026-05-13: PM2 read-only audit found `luker` online on port 8000 from `/opt/Luker/server.js`; Luker third-party extensions live under `/opt/Luker/public/scripts/extensions/third-party/`.
...
- 2026-05-13: Mounted the Minimal TT-Sync server with PM2 only, no Docker and no real App runtime probing. `manual-cloud-tt-sync` listens on `127.0.0.1:8787` and `manual-cloud-tt-sync-https` proxies self-signed HTTPS on Tailscale IP `100.108.182.88:9443`; `curl -kfsS https://100.108.182.88:9443/v2/status` returns `{"ok":true,"service":"minimal-tt-sync","version":"1.0.0"}`.
- 2026-05-13: Tailscale Serve is not enabled for the tailnet yet, so the deployment uses a PM2 HTTPS proxy bound to the Tailscale IP instead of a public URL or Serve-managed MagicDNS HTTPS. `NODE_TLS_REJECT_UNAUTHORIZED=0 timeout 60s npm run smoke:tt-sync-server -- --endpoint https://100.108.182.88:9443 --manifest /tmp/tt-sync-tailscale-smoke-report.json` passed with one 29-byte smoke file.
- 2026-05-13: Generated the current TauriTavern pairing URI for the Tailscale endpoint with `url=https://100.108.182.88:9443`, token from `/home/alan/.local/share/manual-cloud-tt-sync/pairing-token.txt`, 10-minute `exp`, and SPKI pin `k-48NPzuMhrXO7mg_pVKfQNMpIUvHIj5fpl2xzNPKkw`.
- 2026-05-13: Diagnosed the second-phone Pull failure on `/v2/plans/d4eb321a-0aba-49e0-a4d2-1258160a61fc/files/ZGVmYXVsdC11c2VyL2JhY2tncm91bmRzL2phcGFuIGNsYXNzcm9vbSBzaWRlLmpwZw` as likely proxy/mobile connection behavior, not missing server data. The path decodes to `default-user/backgrounds/japan classroom side.jpg`; plan and manifest both include it, the stored file is 453,933 bytes, and both local HTTP plus Tailscale HTTPS downloads returned 200 with matching sha256.
- 2026-05-13: Restarted `manual-cloud-tt-sync-https` with `Connection: close`, disabled keep-alive, and request logging. User retried second-phone Pull and reported it was normal afterwards.
- 2026-05-13: Added frontend TT-Sync progress metrics derived from real events: percent, transfer speed, elapsed time, ETA, and current file. Granularity still depends on how often TauriTavern backend emits progress events; if backend emits only every 10 files, the UI cannot invent per-file updates.
- 2026-05-13: Diagnosed the `manual-cloud-tt-sync` 300MB+ RSS as transfer-time Buffer retention. Restarting the PM2 backend dropped RSS back to roughly 60MB while keeping namespace data and pairings intact, so the repo fix focuses on removing whole-file Buffer allocation from single-file PUT/GET routes instead of hiding the symptom.
- 2026-05-13: Reworked Minimal TT-Sync server single-file transfer to stream uploads into atomic temp files with size/sha256 validation and stream downloads from disk to the response. Bundle JSON endpoints remain compatible and still use base64 JSON by design, but the TauriTavern Push/Pull per-file path no longer buffers the whole file in route memory.
- 2026-05-13: Added regression coverage for streamed single-file transfer and reran concurrent upload coverage. Initial validation exposed a real lost-update bug after moving file transfer outside the plan lock; fixed it by rereading staged metadata inside the short locked plan update.
- 2026-05-13: Resumed for the explicit objective "開始實作 帳號式同步服務 詳細看Readme 需要測試則使用Luker". README and plan show server-side account endpoints exist, but the product path still lacks an account-authenticated way to mint app pairing URIs and the plugin has no account login/device/history panel.
- 2026-05-13: Added `POST /v2/account/pairing-uri`, backed by account bearer auth. It mints a 10-minute one-time TauriTavern pairing URI, consumes that token on `/v2/pair/complete`, avoids duplicating the raw token in the JSON response, and exposes CORS headers for the Luker-hosted plugin panel.
- 2026-05-13: Added a `帳號式同步服務` panel to the TT-Sync settings UI. It can login, refresh token, create a pairing URI, fill the existing `配對 URI` field, and list devices/history through Minimal server HTTP APIs.
- 2026-05-14: Validation passed for the account-style sync phase: `npm run check`, `timeout 60s npm test -- --runInBand`, `git diff --check`, file-size scan, and JS function-length scan. Luker static validation confirmed `/scripts/extensions/third-party/ManualCloudSyncPluginPlan/settings.html` includes the account panel and `modules/tt-sync-account.js` is served. Current PM2 `manual-cloud-tt-sync` has `TT_SYNC_ACCOUNT_USERNAME` and `TT_SYNC_ACCOUNT_PASSWORD` missing, so live account login was not claimed.
- 2026-05-14: Committed the account-style sync service phase with message `feat(sync): add account pairing flow`.
